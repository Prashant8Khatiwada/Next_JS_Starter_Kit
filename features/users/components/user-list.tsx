...
export function UserList() {
  ...
  // Filters
  const { filters, setFilter, resetFilters, getQueryParams } = useFilters<UserFilters>({
    initialFilters: {
      search: "",
      role: "",
      status: "",
  ...
  // Pagination
  const { page, pageSize, totalPages, goToPage, changePageSize, paginationParams } = usePagination({
    initialPage: 1,
    initialPageSize: 5,
  })

  // Users data with filtering and pagination
  const { users, total, isLoading, addUser, updateUser, deleteUser, error, fetchUsers } = useUsers()
      ...
      <Paper shadow="xs" p="md" withBorder className="mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Select
            ...
            value={filters.role}
            onChange={(value) => setFilter("role", value as UserFilters["role"])}
            data={[
          ...
          <Select
            ...
            onChange={(value) => setFilter("status", value as UserFilters["status"])}
            data={[
        ...
        <Group position="apart">
          <Button variant="subtle" leftIcon={<Filter size={16} />} onClick={resetFilters}>
            Reset Filters
          </Button>
                ...
                {users.length === 0 ? (
                  ...
                  users.map((user) => (
                    <tr key={user.id}>
                      <td>
                        <Group spacing="sm">
                          <Avatar size={30} radius={30} src={`https://i.pravatar.cc/150?u=${user.id}`} />
