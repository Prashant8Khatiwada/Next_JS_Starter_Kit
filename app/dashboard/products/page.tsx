...
import { Search, Filter, Plus, Edit, Trash, Eye, MoreHorizontal } from "lucide-react"
    ...
    <DashboardLayout>
      ...
      <Paper withBorder p="md" radius="md" mb="lg">
        <Group position="apart" mb="md">
          ...
          <Button variant="subtle" leftSection={<Filter size={16} />} onClick={resetFilters}>
            Reset
          </Button>
        </Group>
            ...
            {displayedProducts.length === 0 ? (
              ...
              displayedProducts.map((product) => (
                <Table.Tr key={product.id}>
                  ...
                  <Table.Td>
                    <Group gap={4}>
                      <ActionIcon variant="subtle" color="blue" onClick={() => alert(`View product: ${product.title}`)}>
                        <Eye size={16} />
                      </ActionIcon>
                      <ActionIcon variant="subtle" onClick={() => alert(`Edit product: ${product.title}`)}>
                        <Edit size={16} />
                      </ActionIcon>
                      <ActionIcon
                        variant="subtle"
                        color="red"
                        onClick={() => alert(`Delete product: ${product.title}`)}
                      >
                        <Trash size={16} />
                      </ActionIcon>
                      <Menu position="bottom-end" withinPortal>
                        ...
                        <Menu.Dropdown>
                          <Menu.Item onClick={() => alert(`Duplicate product: ${product.title}`)}>Duplicate</Menu.Item>
                          <Menu.Item onClick={() => alert(`Archive product: ${product.title}`)}>Archive</Menu.Item>
                        </Menu.Dropdown>
                      </Menu>
                    </Group>
                  </Table.Td>
      ...
      <Group position="apart" mt="lg">
        ...
        <Pagination total={Math.ceil(filteredProducts.length / pageSize)} value={page} onChange={setPage} withEdges />
      </Group>
    </DashboardLayout>
  )
}
