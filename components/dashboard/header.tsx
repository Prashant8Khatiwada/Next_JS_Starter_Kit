...
import { Avatar, Menu, Text, UnstyledButton, Group, TextInput, ActionIcon, Box } from "@mantine/core"
import { ChevronDown, User, Settings, LogOutIcon as Logout, Bell, Search } from "lucide-react"
...
export function DashboardHeader({ sidebarWidth = 250 }: { sidebarWidth?: number }) {
  ...
  return (
    ...
    >
      ...
      <Group>
        <ActionIcon variant="subtle" radius="xl" size="lg" aria-label="Notifications">
          <Bell size={20} />
        </ActionIcon>
        ...
        >
          <Menu.Target>
            <UnstyledButton
              ...
              style={{
                backgroundColor: userMenuOpened ? "var(--mantine-color-gray-1)" : "transparent",
              }}
            >
              <Group gap={7}>
                <Avatar src={`https://i.pravatar.cc/150?u=${user?.id}`} alt={user?.name} radius="xl" size={30} />
