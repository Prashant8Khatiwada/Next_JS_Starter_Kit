...
import { Badge, Group, Text, TextInput, Select, RangeSlider, Button, Avatar } from "@mantine/core"
  ...
  const columns: DataTableColumn<Product>[] = [
    {
      ...
      render: (product) => (
        <Group spacing="sm">
          {product.image && <Avatar src={product.image} size={30} radius="xl" />}
    ...
    },
    {
      key: "price",
      title: "Price",
      render: (product) => <Text weight={500}>${product.price.toFixed(2)}</Text>,
      sortable: true,
    },
    {
      key: "stock",
      title: "Stock",
      render: (product) => <Text color={product.stock === 0 ? "red" : undefined}>{product.stock}</Text>,
      sortable: true,
    },
    {
      key: "status",
      title: "Status",
      render: (product) => <Badge color={product.status === "active" ? "green" : "red"}>{product.status}</Badge>,
      sortable: true,
    },
  ]
